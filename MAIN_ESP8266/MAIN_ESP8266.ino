...
#include <avr/power.h>
 ...
 //sometimes the LCD adress is not 0x3f. Change to 0x27 if it dosn't work.
...
//////////////////////////////////////////////
const char SSID_ESP[] = "Dilan's Galaxy Note20 Ultra 5G";         //Give EXACT name of your WIFI
const char SSID_KEY[] = "";             //Add the password of that WIFI connection
const char* host = "disenomecatronicorobot1.000webhostapp.com";  //Add the host without "www" Example: electronoobs.com
...
void loop(){
  ...
  // Lectura del sensor 0
  lectura = analogRead(Potentiometer_1);
  sent_nr_1 = pow(3027.4 / lectura, 1.2134); // Envía la distancia calculada en cm
  
  // Lectura del sensor 1
  lectura = analogRead(Potentiometer_2);
  sent_nr_2 = pow(3027.4 / lectura, 1.2134); // Envía la distancia calculada en cm
  
  // Lectura del sensor 2
  lectura = analogRead(Potentiometer_3);
  sent_nr_3 = pow(3027.4 / lectura, 1.2134); // Envía la distancia calculada en cm
